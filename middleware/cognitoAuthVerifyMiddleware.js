...
function _verifyMiddleWare(pemsDownloadProm, req, res, next) {
    pemsDownloadProm.then((pems) => {
        ...
        .catch((err) => {
            const status = 500 // Either not auth or internal error
...
function _verifyProm(pems, auth) {
    return new Promise((resolve, reject) => {
        ...
        if (!auth || auth.length < 10) {
            reject(new Error('Invalid or missing Authorization header. Expected to be in the format \'Bearer <your_JWT_token>\'.'))
            return
        }
        const authPrefix = auth.substring(0, 7).toLowerCase()
        if (authPrefix !== 'bearer ') {
            reject(new Error('Authorization header is expected to be in the format \'Bearer <your_JWT_token>\'.'))
            return
        }
        ...
        if (!decodedNotVerified) {
            ...
            reject(new Error('Authorization header contains an invalid JWT token.')) // don't return detailed info to the caller
            return
        }
